# Encargo · Reposición urgente

## Qué se necesita

| Parámetro               | Valor                                                                  |
| ----------------------- | ---------------------------------------------------------------------- |
| Producto                | Monitor 24 pulgadas Full HD                                            |
| Cantidad                | 40 unidades                                                            |
| Plazo máximo de entrega | **10 días hábiles**                                                    |
| Presupuesto tope        | **USD 7.000,00** puestos en bodega                                     |
| Garantía                | La estándar del proveedor es suficiente. **No se requiere extensión.** |

## Por qué

Farmacéutica Vitalis ordenó 40 monitores con entrega pactada en 12 días. El embarque que cubría
ese pedido se atrasó y ahora llega en 30 días.

El plazo de 10 días hábiles deja margen sobre el compromiso con el cliente. **No es negociable:**
una entrega en 11 días no es "casi a tiempo", es un incumplimiento.

## Cómo se elige entre las que cumplen

Plazo y presupuesto son filtros: una cotización que incumple cualquiera de los dos queda fuera.
**Entre las que sí cumplen, se prefiere el menor total puesto en bodega.**

Sin esta regla, tres proveedores distintos serían respuestas igual de defendibles, y la
comparación no llegaría a una recomendación.

## Qué significa "puesto en bodega"

El presupuesto tope incluye el flete. Un proveedor que cotiza más barato pero cobra el despacho
aparte puede terminar costando más que uno que lo incluye. Comparar precios de lista sin resolver
esto produce una decisión equivocada con números correctos.

## Si un proveedor pide una definición

Algunos proveedores consultan antes de cotizar. Este encargo ya define lo necesario para
responder: cantidad, plazo, presupuesto y garantía. **Si la consulta se responde con lo que está
en este documento, se responde.** Si pidiera algo que no está aquí, se escala a una persona.

## Qué se espera como resultado

Un cuadro comparativo de todas las cotizaciones recibidas, llevadas a una base común, con:

- el precio unitario real de cada proveedor
- el total puesto en bodega para las 40 unidades
- el plazo de entrega
- si cumple o no cada restricción, y por qué
- qué proveedores no respondieron
- una recomendación sustentada con evidencia

**La decisión final la toma una persona.** El sistema no adjudica.
